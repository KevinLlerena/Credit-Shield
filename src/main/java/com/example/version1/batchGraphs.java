...
public class batchGraphs extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_batch_graphs);

        String cardStorage = readFromFile(getApplicationContext());
            Bundle content = getIntent().getExtras();
            String value = content.getString("monthVal");
            int timeLine = Integer.parseInt(value);
            ...
            if (m.find( )) {
                ...
                createGraphs(cardDir,0,dirSize,timeLine);
    ...
    public static int[] batchMode(int[] yearOverview,int minPay,double apr,int bal,int incre,int monthsLeft,int months){
        ...
        double monthBal = (double)bal/monthsLeft;
        if((monthBal+.001) >= ((int)monthBal+0.5))
            monthBal = monthBal + 1;
        int roundmBal = (int)monthBal;
        int newBal = bal + roundMI-roundmBal;
        if(monthsLeft != 0)
            monthsLeft = monthsLeft-1;
        if(newBal < 0)
            newBal = 0;
        yearOverview[incre++] = roundmBal;
        if(incre < months)
            yearOverview = batchMode(yearOverview,minPay,apr,newBal,incre,monthsLeft,months);
        return yearOverview;
    }
    ...
    public void createGraphs(String[] cardList, int num,int listSize, int months){
        ...
        for(int i=num;i<listSize;i++) {
            ...
            int numMinPay = Integer.parseInt(myCardVals[1]);
            int numBal = Integer.parseInt(myCardVals[2]);

            String msg = " error";
            overView = batchMode(overView, numMinPay, numAPR, numBal, 0,months,months);

            int max = returnApex(overView,0);
            int[] overView4 = new int[12];
            overView4 = grabInterest(overView4, numMinPay, numAPR, numBal, 0,months,months);
    ...
    public static int[] grabInterest(int[] yearOverview,int minPay,double apr,int bal,int incre,int monthsLeft,int months){
        ...
        if(incre < months)
            yearOverview = grabInterest(yearOverview,minPay,apr,newBal,incre,monthsLeft,months);
        return yearOverview;
    }
}
